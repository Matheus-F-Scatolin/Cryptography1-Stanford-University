# Week 1 Assignment: Many Time Pad
## Overview
In this assignment, we explore what goes wrong when a stream cipher key is used more than once. You are provided with eleven hex-encoded ciphertexts that are the result of encrypting eleven plaintexts with a stream cipher, all with the same stream cipher key. Your goal is to decrypt the last ciphertext, and submit the secret message within it as the solution.
